...
In this example, we use the `TakePhoto` behavior to take a picture and display it on Jibo's screen. First, we've added a photo element to the skill's `index.html` file (`<div id="photo"></div>`). Then, we use a `StartOnAnimEvent` decorator to take a photo on the exact frame in the `camera.keys` animation when the camera shutter closes. We write the photo's URL to the notepad, and then use an `ExecuteScript` behavior to create an image, set the source of the image to the photo's URL from the notepad, hide Jibo's eye, and display the photo element. After the photo has been displayed on Jibo's screen, we display Jibo's eye again.
